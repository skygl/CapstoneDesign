...
if(!(isset($_COOKIE['teacher']) && $_COOKIE['teacher']==1)){
    echo '선생님만 새로운 선생님과 학생을 등록 할 수 있습니다.';
    $conn->close();
    include('footer.php');
    exit;
}
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="input-group">
                <input type="number" id="session" name="sessionVal" class="form-control" placeholder="수업차시 ex) 1" required>
                <span class="input-group-btn">
                    <input id="submitAttendance" type="button" class="btn btn-success" value="출석체크" name="submitAttendance">
                </span>
            </div>
        </div>
        <div class="col-md-8">
...
if($classes && mysqli_num_rows($classes)){
    ...
    echo '    <option value="">수업을 선택하십시오</option>';
    echo '    <option value="?class=">모든 수업</option>';
